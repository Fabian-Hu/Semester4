...
var saveComment = function() {
    ...
    fetch('http://localhost:8080/studboardRESTReferenz/resources/comment/create', {
...
function loadComment(id, articleId) {
    ...
    if (comment_JSON === null) {
        let path = 'http://localhost:8080/studboardRESTReferenz/resources/comment?id=' + id;
        fetch(path).then(
            function(response) {
...
function loadComments() {
    urlParams = new URLSearchParams(window.location.search);
    fetch('http://localhost:8080/studboardRESTReferenz/resources/comments?articleId=' + urlParams.get("id")).then(
        function(response) {
            let res = response.json();
            return res;
        }
